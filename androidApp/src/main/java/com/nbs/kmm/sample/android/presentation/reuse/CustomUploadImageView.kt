...
import com.nbs.kmm.sample.android.presentation.reuse.camera.CameraActivity.CameraMode
import com.nbs.kmm.sample.android.presentation.reuse.camera.CameraActivity.Companion.PHOTO_REQUEST_CODE
import com.nbs.kmm.sample.android.presentation.reuse.camera.CameraActivity.Companion.startCameraActivityForResult
...
class CustomUploadImageView @JvmOverloads constructor(
    ...
    fun setResultImage(path: String) {
        with(binding) {
            imgResult.visible()
            imgResult.setImageUrl(path,true)
            imgCamera.gone()
